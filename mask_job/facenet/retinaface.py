...
from mask_job.facenet.nets.facenet import Facenet
from mask_job.facenet.nets_retinaface.retinaface import RetinaFace
from mask_job.facenet.utils.anchors import Anchors
from mask_job.facenet.utils.config import cfg_mnet, cfg_re50
from mask_job.facenet.utils.utils import Alignment_1, compare_faces, letterbox_image, preprocess_input
from mask_job.facenet.utils.utils_bbox import (
    decode,
    decode_landm,
    non_max_suppression,
...
class Retinaface(object):
    _defaults = {
        ...
        #   retinaface训练完的权值路径
        # ----------------------------------------------------------------------#
        "retinaface_model_path": os.path.dirname(__file__)+"\\model_data\Retinaface_mobilenet0.25.pth",
        # ----------------------------------------------------------------------#
        ...
        #   facenet训练完的权值路径
        # ----------------------------------------------------------------------#
        "facenet_model_path": os.path.dirname(__file__)+"\\model_data\\facenet_mobilenet.pth",
        # ----------------------------------------------------------------------#
    ...
    def __init__(self, encoding=0,**kwargs):
        ...
        self.sql_class_add = mask_job.sql_class.sql_class_add_people('localhost', 'root', 'root', 'photo', 'photo_list')

        self.sql_class=mask_job.sql_class.sql_class_face('localhost','root','root','face_data','face_list')
        ...
        # try:
        #
        #     with open(os.path.abspath(os.path.join(os.path.dirname(__file__),os.path.pardir))+'\\param.txt','r') as f:
        #         param_list=f.readlines()
        #
        #         self.face_num=int(param_list[1])
        ...
        #
        #
        #     self.known_face_encodings = face_encodings_retrieved1
        #
        ...
        #         os.path.dirname(__file__) + "\\model_data\\{backbone}_ID_encoding.npy".format(backbone=self.facenet_backbone)
        #     )
        #
    ...
    def generate(self):
        ...
        print(self.retinaface_model_path,"-------------------------------------")
    ...
    def encode_face_dataset(self, image_paths, names,ID):
        face_encodings = []
        for index, path in enumerate(tqdm(image_paths)):
            ...
            if self.letterbox_image:
                ...
                anchors = Anchors(
                    self.cfg, image_size=(im_height, im_width)
                ).get_anchors()
            ...
            with torch.no_grad():
                ...
                # -----------------------------------------------------------#
                landms = decode_landm(
                    landms.data.squeeze(0), anchors, self.cfg["variance"]
                )
            ...
            # ---------------------------------------------------#
            crop_img = old_image[
                int(best_face_location[1]) : int(best_face_location[3]),
        ...
        # 清理原来的数据
        self.sql_class.face_delete("encoding")
        self.sql_class.face_delete("o_name")
        self.sql_class.face_delete("o_id")

        # 存储数据至数据库
        self.sql_class.face_insert("encoding",face_encodings)
        name_str=','.join(names)
        id_str=','.join(ID)
        self.sql_class.o_name_inser("o_name",name_str)
        self.sql_class.o_id_inser("o_id",id_str)
    ...
    def detect_image(self, image, camera_name="Unknown"):
        ...
        if self.letterbox_image:
            image = letterbox_image(
                image, [self.retinaface_input_shape[1], self.retinaface_input_shape[0]]
            )
            anchors = self.anchors
        else:
        ...
        for face_encoding in face_encodings:
            ...
            if matches[best_match_index]:
                ...
                id =self.known_names_id[best_match_index]
        ...
        for i, b in enumerate(boxes_conf_landms):
            ...
            cy = b[1] + 12
            # cv2.putText(
            #     old_image, text, (cx, cy), cv2.FONT_HERSHEY_DUPLEX, 0.5, (255, 255, 255)
            # )
                    ...
                    crop_img = (
                        np.array(
                            letterbox_image(
                                np.uint8(crop_img),
                                (
                                    self.facenet_input_shape[1],
                                    self.facenet_input_shape[0],
                                ),
                            )
                        )
                        / 255
    ...
    def create_weight(self,encoding=0):
        try:
            with open(os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.pardir)) + '\param.txt','r') as f:
                param_list = f.readlines()
                self.face_num = int(param_list[1])

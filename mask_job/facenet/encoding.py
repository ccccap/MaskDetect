...
def encoding(sql_class_add):
    ...
    all=sql_class.read_photo_list()         #从数据库取图片 以及个人信息
    image_paths = []
    names = []
    ID=[]
    for i in all:
        with open(os.path.dirname(__file__)+"\\face_dataset\\"+i[0]+".jpg", "wb") as file:  # 存放到某个文件夹进行中转
            file.write(i[4])
    ...
    print(image_paths)
    print(ID)
    retinaface.encode_face_dataset(image_paths,names,ID) # encoding后存入数据库
